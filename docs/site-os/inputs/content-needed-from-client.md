...
| 1.5 | ~~Final logo files (SVG light + SVG dark + favicon source)~~ — **RESOLVED 2026-05-27**: canonical files in `public/images/logos/`. Header + Footer wired to `sirius-systems-star-business-name-cropped-transparent-logo.webp` (1080×635, transparent BG, full star+wordmark lockup). Favicons in `public/images/logos/favicons/` (16, 32, 48, 180, 192, 512 PNG + .ico) wired via `app/layout.tsx` icons block; `favicon.ico` also copied to `public/favicon.ico` for default browser request. **Outstanding:** no SVG-light/inverse variant present (PNG/WEBP only); no OG social card (TODO in `app/layout.tsx`). | Header, footer, favicon, social cards |
